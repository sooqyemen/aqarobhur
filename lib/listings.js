...
function normalizeMediaEntry(entry) {
  ...
  if (typeof entry === 'string') {
    ...
    return {
      ...
      refPath: '',
      name: '',
      kind: looksLikeVideo(url) ? 'video' : 'image',
    };
  }
  ...
  return {
    ...
    kind: kind === 'video' || looksLikeVideo(url) ? 'video' : 'image',
  };
}
...
function deriveMediaFields(payload = {}) {
  const metaFromPayload = normalizeMediaList(payload.imagesMeta);
  if (metaFromPayload.length) {
    const images = metaFromPayload.filter((item) => item.kind !== 'video').map((item) => item.url);
    const videos = metaFromPayload.filter((item) => item.kind === 'video').map((item) => item.url);
    return {
      imagesMeta: metaFromPayload,
      images: dedupeStrings(images),
      videos: dedupeStrings(videos),
    };
  }

  const rawImages = dedupeStrings(payload.images);
  const rawVideos = dedupeStrings(payload.videos);
  const merged = normalizeMediaList([
    ...rawImages.map((url) => ({ url })),
    ...rawVideos.map((url) => ({ url, kind: 'video' })),
  ]);

  return {
    imagesMeta: merged,
    images: merged.filter((item) => item.kind !== 'video').map((item) => item.url),
    videos: merged.filter((item) => item.kind === 'video').map((item) => item.url),
  };
}
...
export async function adminUpdateListing(id, patch) {
  ...
  if (next.imagesMeta || next.images || next.videos) {
