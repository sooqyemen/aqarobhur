...
function listingSortMillis(item = {}) {
  return tsToMillis(item.boostedAt) || tsToMillis(item.updatedAt) || tsToMillis(item.publishedAt) || tsToMillis(item.createdAt);
}

function sortListingsByFreshness(items = []) {
  return [...items].sort((a, b) => listingSortMillis(b) - listingSortMillis(a));
}
...
function normalizeDealKind(value = '') {
...
function normalizePropertyKind(value = '') {
  const v = normalizeComparable(value);
  if (!v) return '';
  if (['ارض', 'اراضي', 'قطعه', 'قطع'].some((x) => v.includes(x))) return 'land';
  if (['شقه', 'شقق', 'دور', 'ادوار'].some((x) => v.includes(x))) return 'apartment';
  if (['فيلا', 'فله', 'فلل', 'فيلات'].some((x) => v.includes(x))) return 'villa';
  if (['عماره', 'عماير', 'عمائر'].some((x) => v.includes(x))) return 'building';
  if (['فندق', 'فنادق', 'فندقي'].some((x) => v.includes(x))) return 'hotel';
...
function sameDealType(actual, wanted) {
  const a = normalizeDealKind(actual);
  const w = normalizeDealKind(wanted);
  return !!a && !!w && a === w;
}

function samePropertyType(item = {}, wanted) {
  const w = normalizePropertyKind(wanted);
  if (!w) return true;
  const hay = [item.propertyType, item.title, item.description].filter(Boolean).join(' ');
  const actualKind = normalizePropertyKind(hay);
  if (actualKind && actualKind === w) return true;
  return sameText(item.propertyType, wanted);
}

function samePropertyClass(actual, wanted, propertyType) {
  const w = String(wanted || '').trim();
  if (!w) return true;
  const raw = String(actual || '').trim() || inferPropertyClass(propertyType);
  const a = normalizeComparable(raw);
  if (w === 'residential') return raw === 'residential' || a.includes('سكني');
  if (w === 'commercial') return raw === 'commercial' || a.includes('تجاري');
  return raw === w || sameText(raw, w);
}
...
function normalizeMediaEntry(entry, forcedKind = '') {
  ...
  const url = String(entry.url || entry.src || entry.href || entry.downloadURL || entry.secure_url || '').trim();
  if (!url) return null;

  const explicitKind = String(forcedKind || entry.kind || entry.type || entry.mediaType || '').trim().toLowerCase();
  let kind = 'image';
...
function collectLegacyMedia(item = {}) {
  ...
  const singleVideos = [item.video, item.videoUrl].filter(Boolean).map((url) => ({ url, kind: 'video' }));
  ...
  return normalizeMediaList([...mixedCollections, ...singleImages, ...singleVideos, ...arrayImages, ...arrayVideos]);
}

function deriveMediaFields(payload = {}) {
  const allMedia = collectLegacyMedia(payload);
  const { images, videos } = splitMediaUrls(allMedia);
  return { imagesMeta: allMedia, images, videos };
}
...
async function getFromBothCollections(getPrimary, getLegacy, { includeLegacy = false } = {}) {
  ...
  if (!includeLegacy) return sortListingsByFreshness(primary);

  try {
    const legacy = await getLegacy();
    return sortListingsByFreshness([...primary, ...legacy]);
  } catch (e) {
    const msg = String(e?.message || '');
    if (e?.code === 'permission-denied' || msg.includes('Missing or insufficient permissions')) return sortListingsByFreshness(primary);
    throw e;
  }
}
...
async function fetchPublicBase(db, max = 200, refOverride) {
  ...
  return sortListingsByFreshness(mapDocs(snap));
}
...
function applyClientFilters(items, filters = {}) {
  ...
  return items.filter((it) => {
    ...
    if (f.neighborhood) {
      const areaHay = [it.neighborhood, it.plan, it.title, it.description].filter(Boolean).join(' ');
      if (!sameText(areaHay, f.neighborhood)) return false;
    }

    if (f.dealType && !sameDealType(it.dealType, f.dealType)) return false;

    if (f.propertyType && !samePropertyType(it, f.propertyType)) return false;

    if (f.plan) {
      const planHay = [it.plan, it.title, it.description].filter(Boolean).join(' ');
      if (!sameText(planHay, f.plan)) return false;
    }

    if (f.part && !sameText(it.part || it.plotNumber, f.part)) return false;

    if (f.q) {
      const q = normalizeComparable(f.q);
      const hay = normalizeComparable(`${it.title || ''} ${it.description || ''} ${it.neighborhood || ''} ${it.plan || ''} ${it.part || ''}`);
      if (!hay.includes(q)) return false;
    }

    if (f.propertyClass && !samePropertyClass(it.propertyClass, f.propertyClass, it.propertyType)) return false;

    const price = normalizeNumber(it.price);
    if (f.priceMin != null && (price == null || price < Number(f.priceMin))) return false;
    if (f.priceMax != null && (price == null || price > Number(f.priceMax))) return false;

    const area = normalizeNumber(it.area);
    if (f.areaMin != null && (area == null || area < Number(f.areaMin))) return false;
    if (f.areaMax != null && (area == null || area > Number(f.areaMax))) return false;
...
export async function fetchLatestListings({ onlyPublic = true, n = 12, includeLegacy = false } = {}) {
  ...
  const primaryFn = async () => {
    ...
    return sortListingsByFreshness(items).slice(0, n);
  };
  ...
  const merged = await getFromBothCollections(primaryFn, legacyFn, { includeLegacy });
  return sortListingsByFreshness(merged).slice(0, n);
}
...
export async function fetchListings({ filters = {}, onlyPublic = true, includeLegacy = false, max = 240 } = {}) {
  ...
  const primaryFn = async () => {
    ...
    return sortListingsByFreshness(applyClientFilters(base, filters));
  };
...
export async function fetchListingById(id, { includeLegacy = true } = {}) {
  ...
  async function tryGetByDocId(colName, docId) {
    try {
      ...
      const msg = String(e?.message || '');
      if (e?.code === 'invalid-argument' || msg.includes('Invalid document reference')) return null;
      throw e;
    }
    return null;
  }
  ...
  if (includeLegacy) {
    try {
      ...
      const msg = String(e?.message || '');
      if (!(e?.code === 'permission-denied' || msg.includes('Missing or insufficient permissions'))) throw e;
    }
  }
...
export async function adminUpdateListing(id, patch) {
  ...
  if (next.propertyType && !next.propertyClass) next.propertyClass = inferPropertyClass(next.propertyType) || 'residential';
  if (next.neighborhood) next.neighborhood = normalizeNeighborhoodName(String(next.neighborhood || '').trim());

  if (
    next.imagesMeta != null || next.images != null || next.videos != null || next.media != null ||
    next.gallery != null || next.photos != null || next.image != null || next.imageUrl != null ||
    next.cover != null || next.video != null || next.videoUrl != null
  ) {
...
export async function adminBoostListing(id) {
  if (!id) throw new Error('معرّف الإعلان مطلوب');
  const { db } = getFirebase();
  await updateDoc(doc(db, COL, id), { boostedAt: serverTimestamp(), updatedAt: serverTimestamp() });
  return { success: true };
}
