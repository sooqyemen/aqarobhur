...
function listingSortMillis(item = {}) {
  return (
    tsToMillis(item.boostedAt) ||
    tsToMillis(item.updatedAt) ||
    tsToMillis(item.publishedAt) ||
...
function normalizeNumber(value) {
  ...
  const n = Number(String(value).replace(/[٠-٩]/g, (d) => '٠١٢٣٤٥٦٧٨٩'.indexOf(d)).replace(/[۰-۹]/g, (d) => '۰۱۲۳۴۵۶۷۸۹'.indexOf(d)).replace(/,/g, ''));
...
async function getFromBothCollections(getPrimary, getLegacy, { includeLegacy = false } = {}) {
  ...
  try {
    ...
    if (e?.code === 'permission-denied' || msg.includes('Missing or insufficient permissions')) {
      return sortListingsByFreshness(primary);
    }
    throw e;
  }
}
...
function applyClientFilters(items, filters = {}) {
  ...
  return items.filter((it) => {
    ...
    if (f.propertyType && !sameText(it.propertyType, f.propertyType)) return false;
