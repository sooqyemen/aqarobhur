...
export function getMediaKind(value) {
  ...
  const lower = getNameLike(value).toLowerCase();
...
export function normalizeMediaEntry(entry, forcedKind = '') {
  ...
  const kind = forcedKind || String(entry.kind || entry.mediaType || '').trim().toLowerCase() || getMediaKind({ ...entry, url });
...
export function getCoverMedia(item = {}) {
  return collectMediaEntries(item)[0] || null;
}
