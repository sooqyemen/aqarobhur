...
function normalizeStatus(value) {
  const allowed = ['internal_ready', 'auto_saved', 'needs_review', 'ignored', 'possible_duplicate', 'expired'];
...
export async function saveInboxEntry(payload) {
  ...
  const ref = await addDoc(collection(db, INBOX_COLLECTION), {
    ...
    status: cleanString(payload.status || 'processed'),
...
export async function saveExtractedItems({ inboxEntryId, items = [] }) {
  ...
  const safeItems = Array.isArray(items)
    ? items.filter((item) => item?.recordType === 'listing' && item?.isFresh !== false)
    : [];
  ...
  for (const item of safeItems) {
    ...
    const nextStatus = isExpiredItem(normalizedItem) ? 'expired' : normalizeStatus(normalizedItem.extractionStatus);
    if (nextStatus === 'expired') continue;

    if (existing) {
      const patch = {
        ...normalizedItem,
        extractionStatus: existing.extractionStatus === 'auto_saved' ? 'auto_saved' : nextStatus,
...
export async function promoteExtractedItem(item) {
  if (item.recordType !== 'listing') {
    throw new Error('الوارد الذكي مخصص لحفظ العروض فقط.');
  }

  if (normalizeStatus(item.extractionStatus) === 'expired' || isExpiredItem(item) || item.isFresh === false) {
    throw new Error('هذا العرض منتهي الصلاحية ولن يُعتمد داخليًا.');
  }
