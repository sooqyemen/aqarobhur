...
import { ensureSourceContact, normalizeSaudiPhone } from './contactUtils';
import { adminCreateListing } from './listings';

export const INBOX_COLLECTION = 'abhur_inbox_entries';
export const EXTRACTED_COLLECTION = 'abhur_extracted_items';
...
function normalizeStatus(value) {
  const allowed = ['auto_saved', 'needs_review', 'ignored', 'possible_duplicate', 'expired', 'sold'];
...
function isExpiredItem(item) {
  if (item?.isFresh === false) return true;
  const expiresAt = normalizeIsoDate(item?.expiresAt);
  if (!expiresAt) return false;
  return new Date(expiresAt).getTime() < Date.now();
}

function isSavableListing(item) {
  if (!item || item.recordType !== 'listing') return false;
  if (isExpiredItem(item)) return false;
  if (normalizeStatus(item.extractionStatus) === 'ignored') return false;
  if (normalizeStatus(item.extractionStatus) === 'sold') return false;
  const listing = item.listing || {};
  return !!(listing.propertyType || listing.area || listing.price || listing.plan || listing.neighborhood);
}
...
export async function saveExtractedItems({ inboxEntryId, items = [] }) {
  ...
  for (const item of items) {
    if (!isSavableListing(item)) continue;

    const source = ensureSourceContact({
      ...(item.source || {}),
      contactPhone: normalizeSaudiPhone(item.source?.contactPhone || ''),
    });
    ...
    const normalizedItem = {
      ...
      duplicateKey,
      retentionDays: Number(item.retentionDays || 30),
      lastSeenAt: nowIso(),
      updatedAt: serverTimestamp(),
    };

    if (existing) {
      const nextStatus = existing.promotedDocId
        ? 'auto_saved'
    ...
    const ref = await addDoc(collection(db, EXTRACTED_COLLECTION), {
      ...
      duplicateOf: '',
      occurrenceCount: 1,
      firstSeenAt: cleanString(normalizedItem.messageDate || nowIso()),
      createdAt: serverTimestamp(),
    });
...
export async function promoteExtractedItem(item) {
  ...
  if (!isSavableListing(item)) {
    throw new Error('هذا العنصر غير صالح للحفظ الداخلي.');
  }

  if (cleanString(item.promotedDocId)) {
    return {
      id: cleanString(item.promotedDocId),
      collection: cleanString(item.promotedCollection || 'abhur_listings'),
    };
  }

  const listingPayload = {
    ...(item.listing || {}),
    title: cleanString(item.listing?.title || item.summary || 'عرض عقاري داخلي'),
    sourceType: source.sourceType,
    sourceContactName: source.contactName,
    sourceContactPhone: source.contactPhone,
    sourceContactRole: source.contactRole,
  ...
  return { id, collection: 'abhur_listings' };
}
