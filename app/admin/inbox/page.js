...
export default function AdminInboxPage() {
  ...
  async function handleAnalyze(payload) {
    ...
    try {
      ...
      setSummary(
        `اكتمل التحليل: تم استخراج ${stats.totalGroups || 0} سجل. (حفظ داخلي تلقائي: ${stats.autoSavedCount || 0} | مراجعة يدوية: ${stats.reviewCount || 0})`
      );
      ...
      const msg = err?.message || 'فشل الذكاء الاصطناعي في تحليل النص.';
      setError(msg);
      throw err;
    } finally {
      setLoading(false);
  ...
  const updateItemLocally = (itemId, newStatus) => {
    setItems((prev) =>
      prev.map((it) =>
        it.id === itemId ? { ...it, extractionStatus: newStatus } : it
      )
    );
  };
  ...
  async function handleDelete(item) {
    if (!window.confirm('حذف نهائي؟ سيتم حذف العرض من الوارد ومن قاعدة البيانات الداخلية.')) {
      return;
    }
                ...
                {recentEntries.map((entry) => (
                  <div key={entry.id} className="entryItem">
                    ...
                    <div className="entrySummary">
                      {entry.aiSummary || 'جاري المعالجة...'}
                    </div>
                  </div>
                ))}

                {!recentEntries.length && (
                  <div className="emptyState">لا توجد سجلات سابقة.</div>
                )}
              </div>
            </div>
          </aside>
  ...
  return (
    <>
      <div className={`statCard ${highlight ? 'pulse' : ''}`}>
        <div className="iconBox" style={{ color, background: `${color}15` }}>
        ...
        <div className="infoBox">
          <div
            className="val"
            style={{ color: highlight ? '#dd6b20' : '#1a202c' }}
          >
            {value}
          </div>
