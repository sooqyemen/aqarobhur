  ...
  useEffect(() => {
    async function fetchOptimizedData() {
      ...
      try {
        ...
        const listingsColl = collection(db, 'abhur_listings');
        const requestsColl = collection(db, 'abhur_requests');
        const extractedColl = collection(db, 'abhur_extracted_items');
        ...
        console.error(err);
        setError('تعذر الاتصال بقاعدة البيانات. تأكد من اتصالك بالإنترنت وصلاحيات حساب الأدمن.');
      } finally {
        setLoading(false);
      }
    }
      ...
      <AdminGuard title="نظرة عامة">
        <AdminShell
          ...
          actions={[
            ...
            <Link key="listings" href="/admin/listings" className="btn secondaryBtn">
                <span className="material-icons-outlined">real_estate_agent</span> إدارة العروض
            </Link>,
                  ...
                  recentAiItems.map((item) => {
                    ...
                    return (
                      <div key={item.id} className="listItem">
                        ...
                        <div className="itemContent">
                          <div className="itemTitle" title={item.summary}>{item.summary ? `${item.summary.slice(0, 40)}...` : 'بدون ملخص'}</div>
                          <div className="itemDesc">
